...
class TextCNN():
    ...
    def __init__(self,W_list,shuffer_falg, static_falg, filter_numbers, filter_sizes,sentence_length,embedding_size,learnrate, epochs, batch_size, dropout_pro, L2_lambda = 0.1):
        ...
        with self.train_graph.as_default():
            ...
            with tf.name_scope('dropout_layer'):
                ...
                max_num = len(filter_sizes) * self.filter_numbers   # 300
    ...
    def __add_conv_layer(self,filter_size,filter_num):
        with tf.name_scope('conv-maxpool-size%d'%(filter_size)):
            ...
            #参数说明
            #第一个参数input：指需要做卷积的输入图像 [训练时一个batch的图片数量（句子的数量）, 图片高度（每个句子中的单词数量）, 图片宽度（每个单词的词向量维度）, 图像通道数（不同方式训练出来的词向量的表示方法，Glove、word2vec等）]
            #第二个参数filter：相当于CNN中的卷积核 [卷积核的高度，卷积核的宽度，图像通道数，卷积核个数]
            #第三个参数strides：卷积时在图像每一维的步长，这是一个一维的向量，长度4,
            #第四个参数padding：string类型的量，只能是"SAME","VALID"其中之一，这个值决定了不同的填充方式
